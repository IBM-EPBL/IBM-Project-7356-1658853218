    ...
    if request.method == "POST":
        ...
        if choice == 'Donor':
            ...
            if account:
                ...
                return render_template("pdashboard.html",choice=choice,data = account,length=len(account),imgname=imgname)
            else:
                sql = "SELECT * FROM PD WHERE email =?"
                stmt = ibm_db.prepare(conn, sql)
                ibm_db.bind_param(stmt,1,email)
...
def dashboard():
    ...
    if request.method == "POST":
        ...
        if choice == "Donor":
            return render_template("dashboard.html", data=data,imgname=imgname)
